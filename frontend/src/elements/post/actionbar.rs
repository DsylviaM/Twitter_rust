...
use crate::{elements::post::quick_respond::QuickRespond, prelude::*, util::api_client};
use dioxus::prelude::*;
use uchat_domain::ids::PostId;
use uchat_endpoint::post::types::LikeStatus;

use super::quick_respond;
...
pub fn LikeDislike(
    ...
    let like_onclick = async_handler!(
        ...
        move |like_status| async move {
            ...
            let like_status = {
                if post_manager
                    .read().get(&post_id).unwrap().like_status == like_status {
            ...
            let request = React { like_status, post_id };
    ...
    let bookmark_onclick = async_handler!(
        ...
        move |_| async move {
            ...
            match fetch_json!(<BookmarkOk>, api_client, request) {
                Ok(res) => {
                    post_manager.write().update(post_id, |post| {
                        post.bookmarked = res.status.clone().into(); //добавила clone() без этого не работало
                    });
                }
                Err(e) => toaster.write().error(
